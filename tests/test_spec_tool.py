...
GOOD_TASKS = "\n".join(
    [
        ...
        "",
        *[
            f"- TASK-{i:02d}: Implement feature {i} with tests and docs "
            f"covering the behaviour in enough detail to be unambiguous."
            for i in range(1, 11)
        ],
    ]
)
...
GOOD_DOD = "\n".join(f"- [ ] Criterion {i}" for i in range(1, 25))
...
def test_tasks_rejects_too_few_ids():
    content = "Tasks:\n- TASK-01 do a thing\n- TASK-02 do another thing\n" + "x" * 400
    with pytest.raises(ValueError, match="at least 8"):
        validate_section_content("tasks", content)


def test_tasks_counts_unique_ids_only():
    # Repeating TASK-01 ten times should NOT satisfy the ≥8 requirement.
    content = ("- TASK-01 do a thing\n" * 10) + ("x" * 400)
    with pytest.raises(ValueError, match="at least 8"):
        validate_section_content("tasks", content)
...
def test_manager_resolver_returns_fresh_store_per_call():
    ...
    first = _resolve_tools(["submit_spec_section"])
    second = _resolve_tools(["submit_spec_section"])
